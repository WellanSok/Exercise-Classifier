...
def merge(arr1,arr2):
	...
	i = 0
	while i<len(arr1):
		newElement = copy.deepcopy(arr1[i])
		for value in arr2[i]:
	...
	if str(request.method) == "POST":
		...
		for d in data['payload']:
			...
			if (sensor_name == "gyroscope"):  
					...
					gyro_sensor_data.append([d["values"]["x"], d["values"]["y"], d["values"]["z"]])
					if len(gyro_sensor_data) > MAX_DATA_POINTS:
						gyro_sensor_data.pop(0)
			...
			if len(all_sensor_data) > MAX_DATA_POINTS:
				all_sensor_data.pop(0)
			all_sensor_data = merge(sensor_data,gyro_sensor_data)
